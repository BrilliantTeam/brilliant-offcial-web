    ...
    if (name.length > 2 && name.length < 23) {
      ...
      .then(data => {
        if (data.data?.userExists) {
          ...
          $('#preserve-minecraft-name').text('你似乎沒有進入過輝煌伺服器，請加入過至少一次再 FILL THIS FIELD，也可能是你「遊戲ID」寫錯了。');
    ...
    if (name.length > 1 && name.length < 41) {
      ...
      .then(data => {
        if (data.data?.userExists) {
          ...
          $('#preserve-discord-tag').text('你似乎沒有進入過輝煌 Discord 群組，請加入後再 FILL THIS FIELD，也可能是你「Discord 使用者名稱」寫錯了。');
