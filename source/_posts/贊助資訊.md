...
window.onload = function() {
  calculateWatercress = () => {
    ...
    if (amount.length > 0) {
      ...
      if (amountInNum >= 170 && amountInNum < 470) {
        watercress += 200;
      } else if (amountInNum >= 470 && amountInNum < 970) {
        watercress += (200 + 580);
      } else if (amountInNum >= 970 && amountInNum < 1170) {
        watercress += (200 + 580 + 1210);
      } else if (amountInNum >= 1170 && amountInNum < 1970) {
        watercress += (200 + 580 + 1210 + 1590);
      } else if (amountInNum >= 1970 && amountInNum < 2470) {
        watercress += (200 + 580 + 1210 + 1590 + 2620);
      } else if (amountInNum >= 2470 && amountInNum < 2970) {
        watercress += (200 + 580 + 1210 + 1590 + 2620 + 2820);
      } else if (amountInNum >= 2970 && amountInNum < 3470) {
        watercress += (200 + 580 + 1210 + 1590 + 2620 + 2820 + 3270);
      } else if (amountInNum >= 3470 && amountInNum < 3970) {
        watercress += (200 + 580 + 1210 + 1590 + 2620 + 2820 + 3270 + 3870);
      } else if (amountInNum >= 3970) {
        watercress += (200 + 580 + 1210 + 1590 + 2620 + 2820 + 3270 + 3870 + 4470);
      }

        $('#Watercress-output').text(` <font color="#e6bbf6">水芥：${watercress} 個</font>（依照輸入金額轉換）`);
        if ($('#donate-amount').hasClass('border-danger')) $('#donate-amount').removeClass('border-danger');
        if (!$('#donate-amount').hasClass('border-success')) $('#donate-amount').addClass('border-success');
        $('#preserve-amount').text('此「金額」的範圍在 30 ~ 10000 之間，請放心填寫其他欄位。');
        $('#preserve-amount').attr('style', 'color: green');
      } else {
        $('#Watercress-output').text('請輸入有效數字');
        $('#Watercress-output').attr('style', 'color: red');
        if ($('#donate-amount').hasClass('border-success')) $('#donate-amount').removeClass('border-success');
        if (!$('#donate-amount').hasClass('border-danger')) $('#donate-amount').addClass('border-danger');
        $('#preserve-amount').text('此「金額」的範圍不在 30 ~ 10000 之間，請確認填寫金額是否過少或過多。');
        $('#preserve-amount').attr('style', 'color: red');
      }
    } else {
      if ($('#Watercress-output').text().length) $('#Watercress-output').text('');
      if ($('#donate-amount').hasClass('border-success')) $('#donate-amount').removeClass('border-success');
      if ($('#donate-amount').hasClass('border-danger')) $('#donate-amount').removeClass('border-danger');
