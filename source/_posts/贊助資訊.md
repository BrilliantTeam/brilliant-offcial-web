  ...
  calculateWatercress = () => {
    ...
    if (amount.length > 0) {
      ...
      if (!isNaN(amountInNum) && amountInNum >= 30 && amountInNum <= 10000) {
        ...
        calculationProcess = `水芥計算過程：${amountInNum} (本次金額)`;
        ...
        for (let i = 0; i < bonuses.length; i++) {
          if (totalAmount >= bonuses[i] && cumulativeInNum < bonuses[i]) {
            ...
            calculationProcess += ` + ${increments[i]} (達到 ${bonuses[i]} 元獎勵)`;
          }
        }

        calculationProcess += ` = ${watercress} 個`;
        $('#Watercress-output').text(`${calculationProcess}`).css('color', '#e6bbf6');
