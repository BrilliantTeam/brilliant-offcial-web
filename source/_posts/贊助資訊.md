    ...
    if (name.length > 2 && name.length < 23) {
      ...
      fetch(`https://api.minecraftservices.com/minecraft/profile/lookup/name/${encodeURIComponent(name)}`, {
        ...
        .then(res => {
          ...
          if (!res.ok) {
            $('#preserve-minecraft-name').text(`UUID 查詢失敗，狀態碼: ${res.status}`);
            $('#minecraftName').removeClass('border-success').addClass('border-danger');
            $('#preserve-minecraft-name').attr('style', 'color: red');
            minecraftAccountStatus = false;
            return Promise.reject();
        ...
        .then(data => {
          ...
          fetch('https://api.brilliantw.net/api/v1/proxy', { 
            ...
            .then(res => {
              if (!res.ok) {
                $('#preserve-minecraft-name').text(`伺服器檢查失敗，狀態碼: ${res.status}`);
                $('#minecraftName').removeClass('border-success').addClass('border-danger');
                $('#preserve-minecraft-name').attr('style', 'color: red');
                minecraftAccountStatus = false;
                return Promise.reject();
            ...
            .then(serverData => {
              if (serverData.data?.userExists) {
                ...
                // 步驟 3: 查詢贊助金額
                fetch(`https://donation-api.brilliantw.net/api/Donation/QueryUserDonation?uuid=${targetUUID}`)
                  .then(res => {
                    if (!res.ok) {
                      $('#preserve-minecraft-name').text(`贊助金額查詢失敗，狀態碼: ${res.status}`);
                      $('#minecraftName').removeClass('border-success').addClass('border-danger');
                      $('#preserve-minecraft-name').attr('style', 'color: red');
                      minecraftAccountStatus = false;
                      return Promise.reject();
                  ...
                  .then(donationData => {
                    if (donationData.success && donationData.donationInfo) {
                      $('#cumulative-amount').val(donationData.donationInfo.totalDonationPaidAmount || 0);
                      calculateWatercress();
                    } else {
                      $('#cumulative-amount').val('0');
                      calculateWatercress();
                    }
                  })
                  .catch(() => {
                    $('#preserve-minecraft-name').text('贊助金額查詢失敗，請稍後再試。');
                    $('#minecraftName').removeClass('border-success').addClass('border-danger');
                    $('#preserve-minecraft-name').attr('style', 'color: red');
                    minecraftAccountStatus = false;
                  });
            ...
            .catch(() => {
              $('#preserve-minecraft-name').text('伺服器檢查失敗，請稍後再試。');
              $('#minecraftName').removeClass('border-success').addClass('border-danger');
              $('#preserve-minecraft-name').attr('style', 'color: red');
              minecraftAccountStatus = false;
            });
        ...
        .catch(() => {
          $('#preserve-minecraft-name').text('UUID 查詢失敗，請檢查網路或稍後再試。');
          $('#minecraftName').removeClass('border-success').addClass('border-danger');
          $('#preserve-minecraft-name').attr('style', 'color: red');
          minecraftAccountStatus = false;
        });
