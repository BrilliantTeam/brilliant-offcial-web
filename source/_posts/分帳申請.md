...
- 此方法暫不可被使用，請在<a href="https://discord.com/invite/5MHGpAFGEN">Discord群組 事務頻道</a>進行申請。
...
window.onload = function() {
  ...
  formSubmit = () => {
      ...
      if (!document.getElementById("comment").val().length) {
      ...
      .then(data => {
          if (data.status) {
              alert(`已您的申請資料已提交至輝煌管理團隊,請等待Discord私訊通知,
倘若沒有接受到通知，請確認伺服器設置中的 允許私人訊息 是否開啟。`);
          } else {
